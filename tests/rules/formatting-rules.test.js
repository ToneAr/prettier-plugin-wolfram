...
describe('newlines-between-definitions', () => {
  it('requires the blank line above a leading comment block, not between the comment and the definition', () => {
    ...
    const node = {
      type: 'ContainerNode',
      children: [
        { type: 'LeafNode', kind: 'Token`Comment', source: [[1, 1], [1, 10]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 10], [2, 1]] },
        { type: 'BinaryNode', op: 'Set', source: [[2, 1], [2, 6]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 6], [3, 1]] },
        { type: 'LeafNode', kind: 'Token`Comment', source: [[3, 1], [3, 10]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[3, 10], [4, 1]] },
        { type: 'BinaryNode', op: 'Set', source: [[4, 1], [4, 6]] },
      ],
    };
  ...
  it('does not count a leading comment block as the separating blank line when there is already a blank line above it', () => {
    ...
    const node = {
      type: 'ContainerNode',
      children: [
        { type: 'BinaryNode', op: 'Set', source: [[1, 1], [1, 6]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 6], [2, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 1], [3, 1]] },
        { type: 'LeafNode', kind: 'Token`Comment', source: [[3, 1], [3, 10]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[3, 10], [4, 1]] },
        { type: 'BinaryNode', op: 'Set', source: [[4, 1], [4, 6]] },
      ],
  ...
  it('requires leading comments to sit directly on top of the following statement', () => {
    ...
    const node = {
      ...
      children: [
        ...
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 10], [2, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 1], [3, 1]] },
        { type: 'BinaryNode', op: 'Set', source: [[3, 1], [3, 6]] },
      ],
    };

    newlinesRule.visit(node, ctx);
    expect(ctx.reports).toHaveLength(1);
    expect(ctx.reports[0].message).toMatch(/Expected 0 blank lines between a leading comment block/);
  });
  ...
  it('allows configured blank lines between non-declaration statements', () => {
    ...
    const node = {
      type: 'ContainerNode',
      children: [
        { type: 'CallNode', source: [[1, 1], [1, 8]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 8], [2, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 1], [3, 1]] },
        { type: 'CallNode', source: [[3, 1], [3, 8]] },
      ],
    };
  ...
  it('caps blank lines between non-declaration statements', () => {
    ...
    const node = {
      ...
      children: [
        ...
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 8], [2, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 1], [3, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[3, 1], [4, 1]] },
        { type: 'CallNode', source: [[4, 1], [4, 8]] },
      ],
    };
  ...
  it('treats same-line comments as trailing on the previous statement', () => {
    ...
    const node = {
      ...
      children: [
        { type: 'CallNode', source: [[1, 1], [1, 8]] },
        { type: 'LeafNode', kind: 'Whitespace', source: [[1, 8], [1, 9]] },
        { type: 'LeafNode', kind: 'Token`Comment', source: [[1, 9], [1, 20]] },
        { type: 'LeafNode', kind: 'Newline', source: [[1, 20], [2, 1]] },
        { type: 'CallNode', source: [[2, 1], [2, 8]] },
      ],
    };
  ...
  it('uses the configured blank lines between adjacent definitions', () => {
    ...
    const node = {
      ...
      children: [
        ...
        { type: 'LeafNode', kind: 'Token`Newline', source: [[1, 6], [2, 1]] },
        { type: 'LeafNode', kind: 'Token`Newline', source: [[2, 1], [3, 1]] },
        { type: 'BinaryNode', op: 'SetDelayed', source: [[3, 1], [3, 7]] },
      ],
    };
...
describe('spacing-operators', () => {
  ...
  it('warns on truly missing operator spaces', () => {
    ...
    spacingRule.visit(node, ctx);
    expect(ctx.reports).toHaveLength(1);
    expect(ctx.reports[0].message).toMatch(/Expected spaces around operator/);
  });
  ...
  it('warns when tight binary operators are spaced out', () => {
    ...
    spacingRule.visit(node, ctx);
    expect(ctx.reports).toHaveLength(1);
    expect(ctx.reports[0].message).toMatch(/Expected no spaces around operator/);
  });
  ...
  it('does not warn on compact span operators', () => {
    ...
    for (const [label, children] of cases) {
      const ctx = makeContext({ wolframSpaceAroundOperators: true });
      spacingRule.visit({ type: 'BinaryNode', op: 'Span', children }, ctx);
  ...
  it('does not report direct-comment operator nodes that the formatter preserves', () => {
    ...
    const node = {
      ...
      children: [
        { type: 'LeafNode', kind: 'Integer', value: '1' },
        { type: 'LeafNode', kind: 'Whitespace', value: ' ' },
        { type: 'LeafNode', kind: 'Token`Comment', value: '(* keep *)' },
        { type: 'LeafNode', kind: 'Whitespace', value: ' ' },
        { type: 'LeafNode', kind: 'Token`SemiSemi', value: ';;' },
        { type: 'LeafNode', kind: 'Whitespace', value: ' ' },
        { type: 'LeafNode', kind: 'Integer', value: '3' },
  ...
  it('ignores trailing comments inside compound expressions', () => {
    ...
    const node = {
      ...
      children: [
        ...
        { type: 'LeafNode', kind: 'Token`Semi', value: ';' },
        { type: 'LeafNode', kind: 'Whitespace', value: ' ' },
        { type: 'LeafNode', kind: 'Token`Comment', value: '(* keep me *)' },
      ],
    };
...
describe('line-width', () => {
  it('warns on lines exceeding printWidth', () => {
    const ctx = makeContext({ printWidth: 10, __sourceText: 'short\nthis line is too long\n' });
  ...
  it('does not warn when a trailing comment exceeds printWidth but the code does not', () => {
    const ctx = makeContext({
      printWidth: 10,
      __sourceText: 'x = 1 (* this trailing comment is intentionally very long *)\n',
    });
    lineWidthRule.visit({ type: 'ContainerNode' }, ctx);
    expect(ctx.reports).toHaveLength(0);
  });
  ...
  it('still warns when code exceeds printWidth before a trailing comment and highlights only the code overflow', () => {
    ...
    lineWidthRule.visit({ type: 'ContainerNode' }, ctx);
    expect(ctx.reports).toHaveLength(1);
    expect(ctx.reports[0].node.source).toEqual([[1, 11], [1, 13]]);
  });
});
...
describe('no-general-infix-function', () => {
  ...
  it('respects configurable preserved ~f~ infix heads', () => {
    const ctx = makeContext({ wolframPreserveTildeInfixFunctions: 'Join,CustomOp' });
    const node = {
      type: 'TernaryNode',
      op: 'TernaryTilde',
      children: [
