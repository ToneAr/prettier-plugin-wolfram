...
function rule(lhs, rhs, op = 'Rule') {
  return {
    ...
    children: [
      lhs,
      leaf(op === 'RuleDelayed' ? 'Token`ColonGreater' : 'Token`MinusGreater', op === 'RuleDelayed' ? ':>' : '->'),
      rhs,
    ],
  };
}
...
function commaSequence(entries) {
  return {
    type: 'InfixNode',
    op: 'Comma',
    children: entries.flatMap((entry, index) => (index === 0 ? [entry] : [comma(), entry])),
  };
}
...
describe('rule value alignment', () => {
  it('aligns association rule values only when the group breaks', () => {
    ...
    expect(fmt(doc, 80)).toBe('<|"URL" -> 1, "Title" -> 2, "Content" -> 3|>');
