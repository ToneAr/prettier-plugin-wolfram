...
describe('printLeaf', () => {
  it('prints a symbol', () => {
    const node = { type: 'LeafNode', kind: 'Symbol', value: 'x', source: [[1,1],[1,2]] };
    expect(printLeaf(node, opts)).toBe('x');
  });

  it('prints an integer', () => {
    const node = { type: 'LeafNode', kind: 'Integer', value: 42, source: [[1,1],[1,3]] };
    expect(printLeaf(node, opts)).toBe('42');
  });

  it('prints a string literal', () => {
    const node = { type: 'LeafNode', kind: 'String', value: 'hello', source: [[1,1],[1,7]] };
  ...
  it('normalizes multiline comments relative to the opening indent', () => {
    ...
    const result = fmt(printLeaf(node, { ...opts, originalText: source, tabWidth: 2 }), 80);
    expect(result).toBe('(*\n  Some comment\n*)');
  });
  ...
  it('returns empty string for whitespace tokens', () => {
    const node = { type: 'LeafNode', kind: 'Token`Whitespace', value: ' ', source: [[1,1],[1,2]] };
    expect(printLeaf(node, opts)).toBe('');
  });

  it('returns empty string for newline tokens', () => {
    const node = { type: 'LeafNode', kind: 'Token`Newline', value: '\n', source: [[1,1],[1,2]] };
    expect(printLeaf(node, opts)).toBe('');
  });
});
