...
describe("diagnostic range anchoring", () => {
	it("anchors whitespace rule findings to the enclosing semantic expression", () => {
		...
		const range = helpers.diagnosticRangeForFinding(
			vscodeApi,
			document,
			ast,
			finding,
	...
	it("expands whitespace-only ranges to the visible line span", () => {
		...
		const whitespaceRange = new Range(
			new Position(0, 0),
			new Position(0, 2),
		);

		const range = helpers.ensureVisibleRange(
			vscodeApi,
			document,
			whitespaceRange,
		);
	...
	it("keeps visible line-width findings on the reported overflow span", () => {
		const text =
			'foo = StringJoin["this is a deliberately overlong string that should not underline the whole expression"]';
	...
	it("keeps one-line-to-multiline formatter diffs on the changed tail", () => {
		const original =
			'foo = StringJoin["a deliberately long string that will be wrapped by formatting"]';
		const formatted =
			"foo =\n" +
			"  StringJoin[\n" +
