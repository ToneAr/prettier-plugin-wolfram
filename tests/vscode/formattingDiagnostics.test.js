...
describe('formattingDiagnostics', () => {
  ...
  it('ignores comment-only width when classifying formatting hunks', () => {
    const hunk = {
      a: ['x ~f~ y (* this comment makes the raw line very long indeed *)'],
      b: ['f[x, y] (* this comment makes the raw line very long indeed *)'],
      aStart: 0,
      aEnd: 1,
      bStart: 0,
      bEnd: 1,
    ...
    expect(classifyFormattingHunk(hunk, 20)).toBe('Formatting differs from configured Prettier style.');
  });
