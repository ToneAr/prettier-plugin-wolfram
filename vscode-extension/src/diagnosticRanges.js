...
function rangeForHunk(vscodeApi, document, hunk) {
  ...
  if (aLen === 1) {
    ...
    while (
      ...
      oldLine[oldLine.length - 1 - suffix] === newEndLine[newEndLine.length - 1 - suffix]
    ) {
      suffix++;
    }
    const start = new vscodeApi.Position(hunk.aStart, prefix);
    const endChar = Math.max(prefix + 1, oldLine.length - suffix);
    return new vscodeApi.Range(start, new vscodeApi.Position(hunk.aStart, endChar));
  }
  ...
  if (aLen === 0) {
    ...
    return new vscodeApi.Range(
      ...
      new vscodeApi.Position(line, Math.max(1, document.lineAt(line).text.length))
    );
  }
  ...
  if (aLen > 1) {
    ...
    while (
      ...
      endText[endText.length - 1 - suffix] === newEndText[newEndText.length - 1 - suffix]
    ) {
      suffix++;
    }

    const startChar = clamp(prefix, 0, document.lineAt(startLine).text.length);
    const endChar = clamp(
...
function isSemanticDiagnosticNode(node) {
  ...
  if (typeof node.locStart !== 'number' || typeof node.locEnd !== 'number') return false;
...
function findBestDiagnosticNode(root, startOffset, endOffset, best = null) {
  ...
  if (typeof root.locStart !== 'number' || typeof root.locEnd !== 'number') return best;
...
function nodeRange(vscodeApi, document, node) {
  ...
  if ([startLine, startCol, endLine, endCol].some((n) => typeof n !== 'number')) return null;
  const maxLine = Math.max(0, document.lineCount - 1);
  const safeStartLine = clamp(startLine - 1, 0, maxLine);
  const safeEndLine = clamp(endLine - 1, safeStartLine, maxLine);
  const safeStartCol = clamp(startCol - 1, 0, document.lineAt(safeStartLine).text.length);
  const safeEndCol = clamp(endCol - 1, 0, document.lineAt(safeEndLine).text.length);
...
function visibleLineRange(vscodeApi, document, startLine, endLine) {
  ...
  return new vscodeApi.Range(
    ...
    new vscodeApi.Position(safeEndLine, document.lineAt(safeEndLine).text.length)
  );
}
...
function ensureVisibleRange(vscodeApi, document, range) {
  ...
  return visibleLineRange(vscodeApi, document, range.start.line, range.end.line);
}
