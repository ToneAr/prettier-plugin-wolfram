...
const APPLY_DOCUMENT_FORMATTING_COMMAND = 'prettier-wolfram.applyDocumentFormatting';
...
function isPrettierWolframDiagnostic(diagnostic) {
  return typeof diagnostic?.source === 'string' && diagnostic.source.startsWith('prettier-wolfram:');
}

function preferredRange(requestedRange, diagnostics) {
  if (diagnostics.length === 1 && diagnostics[0]?.range) return diagnostics[0].range;
  return requestedRange ?? diagnostics[0]?.range ?? null;
}

function provideFormattingCodeActions(vscodeApi, document, range, context) {
  const diagnostics = (context?.diagnostics ?? []).filter(isPrettierWolframDiagnostic);
