...
try {
	...
	if (!packagedPluginVersion)
		throw new Error("Could not determine packed plugin version");
	...
	if (existsSync(join(extensionBuildDir, "package-lock.json")))
		rmSync(join(extensionBuildDir, "package-lock.json"));
	if (existsSync(join(extensionBuildDir, "bun.lock")))
		rmSync(join(extensionBuildDir, "bun.lock"));
