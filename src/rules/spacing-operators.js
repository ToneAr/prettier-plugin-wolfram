...
export default {
  ...
  description: 'Operator spacing inconsistent with wolframSpaceAroundOperators option',
  defaultLevel: 'warn',
  fixableByFormatter: true,

  visit(node, context) {
    if (node.type !== 'InfixNode' && node.type !== 'BinaryNode' && node.type !== 'TernaryNode') return;
    const children = node.children ?? [];
    if (children.some(isDirectComment)) return;

    const isSpaceTrivia = (child) =>
      child?.type === 'LeafNode' &&
      ['Token`Whitespace', 'Whitespace', 'Token`Newline', 'Newline'].includes(child.kind);
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      const isOp = !ignoredOperatorKinds.has(c.kind) &&
        !['Token`Whitespace', 'Whitespace', 'Token`Newline', 'Newline'].includes(c.kind) &&
        !c.kind.startsWith('Token`Comment') &&
        !c.kind.startsWith('Token`Fake`') &&
        !c.kind.startsWith('Token`Hash');
      if (!isOp) continue;
      const wantSpaces = wantsSpacesAroundOperator(node, context.options, c);
      const prevIsWhitespace = i > 0 && isSpaceTrivia(children[i - 1]);
      const nextIsWhitespace = i < children.length - 1 && isSpaceTrivia(children[i + 1]);
      const ok = wantSpaces ? (prevIsWhitespace && nextIsWhitespace) : (!prevIsWhitespace && !nextIsWhitespace);
      if (!ok) {
        context.report({
          node: c,
          message: wantSpaces
