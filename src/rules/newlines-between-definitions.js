...
export default {
  ...
  description: 'Top-level declarations and preserved code gaps should match configured blank-line spacing; leading comments stay attached to the following statement',
  defaultLevel: 'warn',
  fixableByFormatter: true,

  visit(node, context) {
    if (node.type !== 'ContainerNode') return;
    const children = (node.children ?? []).filter((c) => !isWhitespaceTrivia(c));
    ...
    for (const child of children) {
      ...
      const groupStartLine = pendingLeadingCommentStartLine ?? currStartLine;
      const leadingCommentGap =
        pendingLeadingCommentEndLine == null
          ? 0
          : Math.max(0, currStartLine - pendingLeadingCommentEndLine - 1);
