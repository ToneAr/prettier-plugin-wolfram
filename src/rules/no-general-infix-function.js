...
export default {
  ...
  description: 'Prefer fully qualified call syntax over general infix ~f~ form',
  defaultLevel: 'warn',
  ...
  visit(node, context) {
    ...
    const semantic = (node.children ?? []).filter((c) =>
      !(c.type === 'LeafNode' && ['Token`Whitespace', 'Whitespace', 'Token`Newline', 'Newline'].includes(c.kind))
    );
    if (semantic.length !== 5) return;

    const fn = semantic[2];
    if (fn?.type === 'LeafNode' && fn.kind === 'Symbol' && preservedTildeFunctions(context.options).has(fn.value)) {
      return;
    }
