...
function lhsRhs(node) {
  const semantic = node.children.filter(c => !isTrivia(c) &&
    !(c.type === 'LeafNode' && c.kind.startsWith('Token`') && !isSemanticTokenLeaf(c)));
  return [semantic[0], semantic[1]];
}

function operatorToken(node) {
  const tokens = node.children.filter(c => !isTrivia(c) && isOperatorTokenLeaf(c));
  return tokens.length === 1 ? tokens[0] : null;
}

function printSpan(node, print) {
  const semantic = node.children.filter((c) => !isTrivia(c));
  const tokenIndex = semantic.findIndex((c) => isOperatorTokenLeaf(c) && c.value === ';;');
  if (tokenIndex === -1) return null;
  const lhs = semantic.slice(0, tokenIndex).find((c) => !isOperatorTokenLeaf(c));
  const rhs = semantic.slice(tokenIndex + 1).find((c) => !isOperatorTokenLeaf(c));
...
function isMultilineStringJoin(node) {
  ...
  return argPathEntries(node).filter((entry) =>
    !(entry.node?.type === 'LeafNode' && entry.node.kind === 'Token`Comma')
  ).length > 1;
}

function isMultilineStringLeaf(node, printedDoc) {
  return node?.type === 'LeafNode' && node.kind === 'String' && willBreak(printedDoc);
}
...
export function printBinary(node, options, print) {
  ...
  const rhsWillBreak = isMultilineStringLeaf(rhs, rhsDoc) || isMultilineStringJoin(rhs);
