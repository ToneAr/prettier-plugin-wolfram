...
const BRACKET_KINDS = new Set([
  ...
  'Token`LessBar', 'Token`BarGreater',
]);
...
function sequenceEntries(path, print, node) {
  const contents = node.children.filter((child) => !isTrivia(child) && !isBracketToken(child));

  if (contents.length === 1 && contents[0].type === 'InfixNode' && contents[0].op === 'Comma') {
...
export function printGroup(path, options, print, node) {
  ...
  const entries = withAlignedRuleValues(sequenceEntries(path, print, node), path, options, print);
