...
function flattenStringJoinParts(path, print, node, basePath = []) {
  ...
  for (const entry of argPathEntries(node)) {
    if (entry.node?.type === 'LeafNode' && entry.node.kind === 'Token`Comma') {
      continue;
    }

    const entryPath = [...basePath, ...entry.path];
    if (isStringJoinCall(entry.node)) {
      parts.push(...flattenStringJoinParts(path, print, entry.node, entryPath));
      continue;
    }
...
function stringJoinPartDocs(parts, options, indentDepth) {
  ...
  function flushStrings() {
    ...
    docs.push(...stringLiteralRunDocs(stringRun, options, {
      indentDepth,
      mode: 'stringJoinArg',
    }));
    stringRun = [];
...
function printBlockStructure(path, options, print, node) {
  ...
  if (!varListNode || containsComment(varListNode)) return printCall(path, options, print, node);

  const body = args.slice(1);
  const varListDoc = formatVarListPath(path, options, print, node, varListNode);
...
function _getArgNodesAndPrinted(path, options, print, node) {
  ...
  if (wrapperIdx !== -1) {
    ...
    rawArgs = wrapper.children.filter(c => {
      if (c.type === 'LeafNode' && (c.kind === 'Token`Comma' || c.kind === 'Token`Whitespace' || c.kind === 'Whitespace')) return false;
      return true;
    });
  } else {
    rawArgs = node.children.filter(c => {
      if (c.type === 'LeafNode' &&
          (c.kind === 'Token`OpenSquare' || c.kind === 'Token`CloseSquare' ||
           c.kind === 'Token`Comma' || c.kind === 'Token`Whitespace' || c.kind === 'Whitespace')) return false;
      return true;
    });
  }
...
function semanticArgsGroup(node) {
  return (node.children ?? []).filter(c =>
    !isTrivia(c) &&
    !(c.type === 'LeafNode' &&
      (c.kind === 'Token`OpenCurly' || c.kind === 'Token`CloseCurly' ||
       c.kind === 'Token`LessBar' || c.kind === 'Token`BarGreater' ||
       c.kind === 'Token`Comma'))
  );
}
...
function semanticGroupEntries(callNode, groupNode) {
  ...
  const semanticChildren = (groupNode.children ?? []).filter((child) =>
    !isTrivia(child) &&
    !(child.type === 'LeafNode' &&
      (child.kind === 'Token`OpenCurly' || child.kind === 'Token`CloseCurly' ||
       child.kind === 'Token`LessBar' || child.kind === 'Token`BarGreater'))
  );
  ...
  if (
    ...
    return semanticChildren[0].children.reduce((entries, child, idx) => {
      ...
      if (child.type === 'LeafNode' && child.kind === 'Token`Comma') return entries;
      entries.push({
        node: child,
        path: [...basePath, 'children', commaWrapperIdx, 'children', idx],
      });
      return entries;
    }, []);
  }
...
function inlineNodeText(node) {
  ...
  if (node.type === 'CompoundNode' && (node.op === 'Slot' || node.op === 'SlotSequence')) {
    return (node.children ?? []).map(inlineNodeText).join('');
  }
  if (node.type === 'BinaryNode' && (node.op === 'Set' || node.op === 'SetDelayed')) {
    const semantic = (node.children ?? []).filter(c =>
      !isTrivia(c) && !(c.type === 'LeafNode' && c.kind.startsWith('Token`') && !['Token`Hash', 'Token`HashHash'].includes(c.kind))
    );
    const op = node.op === 'Set' ? '=' : ':=';
    if (semantic.length === 2) return `${inlineNodeText(semantic[0])} ${op} ${inlineNodeText(semantic[1])}`;
  }
  if (node.type === 'BinaryNode') {
    const semantic = (node.children ?? []).filter(c =>
      !isTrivia(c) && !(c.type === 'LeafNode' && c.kind.startsWith('Token`') && !['Token`Hash', 'Token`HashHash'].includes(c.kind))
    );
    const opMap = { Power: '^', Divide: '/', ReplaceAll: '/.', Rule: '->', RuleDelayed: ':>', Condition: '/;' };
    const op = opMap[node.op] ?? node.op;
    if (semantic.length === 2) return `${inlineNodeText(semantic[0])} ${op} ${inlineNodeText(semantic[1])}`;
  }
  if (node.type === 'InfixNode') {
    const semantic = (node.children ?? []).filter(c =>
      !isTrivia(c) && !(c.type === 'LeafNode' && c.kind === 'Token`Comma')
    );
    if (node.op === 'Comma') return semantic.map(inlineNodeText).join(', ');
    if (node.op === 'Plus') return semantic.map(inlineNodeText).join(' + ');
    if (node.op === 'Times') return semantic.map(inlineNodeText).join(' * ');
    if (node.op === 'InfixInequality' && semantic.length === 3) return `${inlineNodeText(semantic[0])} ${inlineNodeText(node.children[1])} ${inlineNodeText(semantic[2])}`;
    if (node.op === 'CompoundExpression') return semantic.map(inlineNodeText).join('; ');
  }
  if (node.type === 'CallNode') {
    const head = inlineNodeText(node.head);
    const args = (node.children ?? []).filter(c =>
      !isTrivia(c) && !(c.type === 'LeafNode' && (c.kind === 'Token`OpenSquare' || c.kind === 'Token`CloseSquare'))
    );
    if (args.length === 1 && args[0].type === 'InfixNode' && args[0].op === 'Comma') {
