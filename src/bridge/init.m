...
normalizeSource[source : {{_Integer, _Integer}, {_Integer, _Integer}}] := source
...
nodeSource[expr_] :=
	Module[{meta, source},
		...
		source =
			Quiet @ Check[meta[CodeParser`Source], Missing["NotAvailable"]];
...
cstToJSON[
	CodeParser`CallNode[headNodes_List, childGroup_CodeParser`GroupNode, meta_]
] :=
	<|
		"type" -> "CallNode",
		"head" -> If[headNodes === {},
			<|"type" -> "Unknown", "wl" -> ""|>,
			cstToJSON[First[headNodes]]
		],
...
getCSTJSON[source_String, tabWidth_ : 2] :=
	Module[{cst, json},
		cst =
			Quiet @
			Check[
				CodeParser`CodeConcreteParse[source, "TabWidth" -> tabWidth],
				$Failed
			];
